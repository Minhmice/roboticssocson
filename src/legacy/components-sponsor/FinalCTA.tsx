...
export default function FinalCTASection() {
  ...
  return (
    <section
      id="final-cta"
      className="relative py-12 sm:py-16 md:py-24 border-t border-primary/20 overflow-hidden"
    >
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute inset-0 bg-gradient-to-r from-primary/5 via-transparent to-transparent" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(37,99,235,0.05),transparent)]" />
      </div>

      <div className="max-w-5xl mx-auto px-4 sm:px-6 md:px-8 relative">
        <AnimatedSection className="text-center">
          <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground mb-3 sm:mb-4 px-2 sm:px-0">
            {locale === "vi"
              ? "Hãy đồng hành cùng chúng em!"
              : "Join us on this journey!"}
          </h3>
          <p className="text-base sm:text-lg text-muted-foreground mt-2 sm:mt-3 mb-6 sm:mb-8 px-2 sm:px-0">
            {locale === "vi"
